...
udp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    uint16_t psum = 0;
    ip_endp_t src, dst;
    char endp1[IP_ADDR_STR_LEN];
    char endp2[IP_ADDR_STR_LEN];
    struct udp_pcb *pcb;
    uint16_t iphdrlen;
    ...
    if (!pcb) {
        ...
        icmp_output(ICMP_TYPE_DEST_UNREACH, ICMP_CODE_PORT_UNREACH, 0, (uint8_t *)iphdr, iphdrlen + 8, iface->unicast, iphdr->dst);
        return;
    }
